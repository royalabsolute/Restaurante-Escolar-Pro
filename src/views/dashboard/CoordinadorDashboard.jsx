...
const CoordinadorDashboard = () => {
    ...
    const panelTitle = 'Panel Coordinador de Convivencia';
    const panelSubtitle = 'Coordina la convivencia escolar, gestiona estudiantes y supervisa asistencia';
    ...
    const loadDashboardData = async () => {
        try {
            ...
            // El coordinador usa los mismos endpoints que secretaria
            if (!endpoints?.students || !endpoints?.justifications) {
                setLoading(false);
                return;
            }

            const [pendingStudentsRes, validatedStudentsRes, justificationsRes] = await Promise.allSettled([
            ...
            if (pendingStudentsRes.status === 'fulfilled' && pendingStudentsRes.value?.data) {
                if (Array.isArray(pendingStudentsRes.value.data.data)) {
            ...
            if (validatedStudentsRes.status === 'fulfilled' && validatedStudentsRes.value?.data) {
                if (Array.isArray(validatedStudentsRes.value.data.data)) {
            ...
            if (justificationsRes.status === 'fulfilled' && justificationsRes.value?.data) {
                if (Array.isArray(justificationsRes.value.data.data)) {
            ...
            console.error('[CoordinadorDashboard] Error loading dashboard data:', error);
    ...
    const quickActions = [
        ...
        },
        {
            ...
            description: 'Análisis detallado de la asistencia',
            icon: <AssessmentIcon sx={{ fontSize: 28 }} />,
            path: '/app/reportes/estadisticas',
            color: '#4A90E2',
            bgColor: '#E3F2FD'
        }
    ];
            ...
            <Box mb={4}>
                <Box display="flex" alignItems="center" gap={1.5} mb={3}>
                    ...
                    <Typography variant="h5" sx={{ fontWeight: 600, color: 'text.primary' }}>
                        Accesos Rápidos de Coordinación
                    </Typography>
                </Box>
